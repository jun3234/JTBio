...
def vcf_merge(vcf1, vcf2, outfile=sys.stdout):
    ...
    for v1_key in v1_d:
        ...
        try:
            ...
            if v1_line.ALT == v2_line.ALT:
                kf = ["GT","AD","DP","GQ","PL"]
                samples_v1 = exact_match_FS(v1_line.FORMAT, v1_line.Samples)
                samples_v2 = exact_match_FS(v2_line.FORMAT, v2_line.Samples)
                txt = list(v1_line[:8]) + [":".join(kf)] + samples_v1 + samples_v2
